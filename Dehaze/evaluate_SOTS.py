...
def proc(filename):
    ...
    tar_name = '/home/ubuntu/zsh/datasets/SOTS/outdoor/gt/' + prd_name
    # print('tar',tar)
    # print('prd',prd)
...
parser = argparse.ArgumentParser(description='Dehazing using AST-v2')
...
for dataset in datasets:
    ...
    gt_path = os.path.join('/home/ubuntu/zsh/datasets/SOTS/outdoor/gt/')
    gt_list = natsorted(glob(os.path.join(gt_path, '*.png')) + glob(os.path.join(gt_path, '*.tif')))
    assert len(gt_list) != 0, "Target files not found"

    
    file_path = os.path.join('/home/ubuntu/zsh/code/AST_CAversion/Dehaze/results', 'AST_256_200', dataset)
